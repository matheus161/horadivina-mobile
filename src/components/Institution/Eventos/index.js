...
export default function News({ institution }) {
  ...
  const fetchData = async (page) => {
    setIsLoading(true);
    try {
      const events = await eventsService.getAllEventsByInstitution(
        institution._id,
        page
      );

      setData((prevData) => [...prevData, ...events.paginatedResults]);
      setDataTotalItens(events.totalitens);
    } catch (error) {
      console.log(error);
      Toast.show({
        type: "error",
        text1: "Ops...",
        text2: "Erro ao carregar os eventos, tente novamente!",
      });
    } finally {
      setIsLoading(false);
    }
  ...
  const emptyListMessage = () => {
    return (
      ...
      >
        <Text style={styles.noResultMesage}>Não há eventos cadastrados.</Text>
        <Image
